...
$(LIB)/assimp.bin: $(ASSIMP_ALL_DEPS) assimp
	...
	cd $(TEMP)/assimp ; make $(MAKE_VERBOSE_STR) -s
	cp -va $(TEMP)/assimp/bin/lib* $(LIB)/
...
$(LIB)/bullet.bin: $(BULLET_ALL_DEPS) bullet
	...
	cd $(TEMP)/bullet ; make $(MAKE_VERBOSE_STR) -s
...
$(LIB)/libdeflate.bin: $(LIBDEFLATE_ALL_DEPS) libdeflate
	@make $(MAKE_VERBOSE_STR) -s common
	@rm -rf $(TEMP)/libdeflate
	$(CMAKE) -G "Unix Makefiles" $(CMAKE_TOOLCHAIN) -B $(TEMP)/libdeflate -S $(PWD)/libdeflate
	cd $(TEMP)/libdeflate ; make $(MAKE_VERBOSE_STR) -s
	cp -va $(TEMP)/libdeflate/libdeflate* $(LIB)/
...
$(LIB)/lua-luajit-compound-operators.bin: $(LUA_ALL_DEPS) lua-luajit-compound-operators
	...
	cd $(TEMP)/lua-luajit-compound-operators/lua52/lua-5.2.4 && make $(MAKE_VERBOSE_STR) -s $(LUA_MAKE)
	cd $(TEMP)/lua-luajit-compound-operators/lua52/lua-5.2.4/src/ && cp -arfv *.a *.dll *.exe $(LIB)/ || :
	cd $(TEMP)/lua-luajit-compound-operators/v2.1.ROLLING/ && make $(MAKE_VERBOSE_STR) -s $(LUAJIT_MAKE)
...
$(LIB)/sdl3-osx.bin:
	...
	cp -raf $(TEMP)/sdl/SDL3/SDL3.xcframework/macos-arm64_x86_64/SDL3.framework $(LIB)
