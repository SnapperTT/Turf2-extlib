...
# Paths for building sub libraries
PWD=$(shell pwd)
BASE=$(PWD)
...
LUAJIT_MAKE=
BGFX_MAKE=make -s linux CC='$(CC)' CXX='$(CXX)' CXX_FLAGS='$(CXXFLAGS)'
...
ifeq ($(TARGET_OS),win)
  ...
  BGFX_MAKE=make -s mingw-gcc-debug64 mingw-gcc-release64 CC='$(CC)' CXX='$(CXX)' AR='$(AR)' LD='$(LD)' CXXFLAGS+='$(CXXFLAGS) -DM_PI=3.14159265358979323846264338327950288 '
endif
...
ifeq ($(IS_OSX),yes)
  ...
  BGFX_MAKE=OSXCROSS=$(OSX_PREFIX) GENIE="../bx/tools/bin/linux/genie --with-macos=$(OSX_DEPLOYMENT_TARGET)" make $(BGFX_TARGET) CC=$(CC) CXX=$(CXX) AR=$(AR) LD=$(LD)
endif
...
define fetch_git_release
	...
	# untar's in temp/
	LATEST=`sh $(PWD)/scripts/fetch_git_release.sh $(2)` ; \
...
# 
ALL_SOURCES=lzz-bin assimp btMultilevelProjectedHeightmap bullet bx bimg bgfx bgfx-header-extension-library concurrentqueue FastString fmt libbacktrace libdeflate lua-luajit-compound-operators nanovg nanovg-command-buffer node-unidecode-cxx quant rapidjson readerwriterqueue sdl-stb-font snappy sol2 stb sttr tsl vecgui xxHash
...
assimp:
	@make -s common
	@rm -rf $(TEMP)/assimp
	@$(call fetch_git_release,assimp,"https://github.com/assimp/assimp")
# patch minizip issue
...
bullet:
	@make -s common
	@rm -rf $(TEMP)/bullet
	@$(call fetch_git_release,bullet,"https://github.com/bulletphysics/bullet3")
# copy missing file
...
concurrentqueue:
	...
	@$(call fetch_git_release,concurrentqueue,"https://github.com/cameron314/concurrentqueue")
...
libdeflate:
	...
	@$(call fetch_git_release,libdeflate,"https://github.com/ebiggers/libdeflate")
...
rapidjson:
	...
	@$(call fetch_git_release,rapidjson,"https://github.com/Tencent/rapidjson")
...
readerwriterqueue:
	...
	@$(call fetch_git_release,readerwriterqueue,"https://github.com/cameron314/readerwriterqueue")
...
snappy:
	@make -s common
	@rm -rf $(TEMP)/snappy
	@$(call fetch_git_release,snappy,"https://github.com/google/snappy")
...
sol2:
	@make -s common
	@rm -rf $(TEMP)/sol2
	@$(call fetch_git_release,sol2,https://github.com/ThePhD/sol2)
...
tsl:
	@make -s common
	@rm -rf $(TEMP)/tsl
	@$(call fetch_git_release,tsl,https://github.com/Tessil/robin-map)
